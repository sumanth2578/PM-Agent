...
export function UserStoriesGenerator() {
    ...
    useEffect(() => {
        ...
        fetchUser();
        setSavedStories(JSON.parse(localStorage.getItem('pm_stories') || '[]'));
    }, []);
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const newItem: SavedStories = { id: Date.now().toString(), feature: featureDesc, content: result, createdAt: new Date().toISOString() };
            const updated = [newItem, ...savedStories];
            setSavedStories(updated);
            localStorage.setItem('pm_stories', JSON.stringify(updated));
    ...
    const handleDownload = () => {
        const blob = new Blob([selectedStory?.content || generatedStories], { type: 'text/markdown' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a'); a.href = url;
        a.download = `UserStories-${(selectedStory?.feature || featureDesc).substring(0, 30).replace(/[^a-zA-Z0-9]/g, '_')}.md`;
        a.click(); URL.revokeObjectURL(url);
    };
    const handleDelete = (id: string) => {
        const updated = savedStories.filter(s => s.id !== id);
        setSavedStories(updated); localStorage.setItem('pm_stories', JSON.stringify(updated));
        ...
        <div className="flex h-screen w-full overflow-hidden bg-[#050505] text-white">
            ...
            <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-[#0B0C10] border-r border-white/10 transform transition-transform duration-300 flex flex-col pt-6 pb-4 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
                ...
                <nav className="flex-1 overflow-y-auto custom-scrollbar py-2"><ul className="space-y-1 px-3">
                    ...
                    <li><Link to="/user-stories" className="flex items-center text-white bg-white/10 font-medium rounded-xl px-3 py-2.5"><Users className="w-5 h-5 mr-3 text-red-500 animate-pulse" /> User Stories</Link></li>
            ...
            <div className="flex-1 flex flex-col md:ml-64 h-screen overflow-hidden relative">
                ...
                <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
                    <div className="absolute w-24 h-16 animate-robot-roam opacity-20" style={{ animationDelay: '8s' }}>
                ...
                <header className="flex items-center justify-between px-4 md:px-10 py-6 bg-[#0B0C10]/80 backdrop-blur-xl border-b border-white/5 z-30 flex-shrink-0">
                    <div><h1 className="text-2xl font-bold text-white tracking-tight flex items-center gap-3"><Users className="w-7 h-7 text-red-500 animate-pulse" />User Stories Generator</h1>
                        <p className="text-gray-400 text-sm mt-1">Generate user stories with acceptance criteria and story points</p></div>
                </header>
