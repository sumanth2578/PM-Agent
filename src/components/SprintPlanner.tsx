...
export function SprintPlanner() {
    ...
    const handleDownload = () => {
        ...
        element.innerHTML = `
            <div style="padding: 40px; font-family: sans-serif; color: #333; line-height: 1.6;">
                <h1 style="color: #ef4444; border-bottom: 2px solid #ef4444; padding-bottom: 10px;">Sprint Plan: ${duration}</h1>
        ...
        const opt = {
            margin: 10,
            filename: `Sprint-Plan-${new Date().toISOString().split('T')[0]}.pdf`,
    ...
    return (
        <div className={`flex h-screen w-full overflow-hidden transition-colors duration-300 ${theme === 'dark' ? 'bg-[#050505] text-white' : 'bg-slate-50 text-slate-900'}`}>
            ...
            <aside className={`fixed inset-y-0 left-0 z-40 w-64 border-r transform transition-transform duration-300 flex flex-col pt-6 pb-4 ${theme === 'dark' ? 'bg-[#0B0C10] border-white/10' : 'bg-white border-slate-200'} ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
