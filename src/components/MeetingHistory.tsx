...
import { Clock, Trash2, Mail, Edit, Save, ChevronRight, ExternalLink, RotateCw } from 'lucide-react';
...
export function MeetingHistory() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await supabase.from('meetings').update({ is_hidden: true }).eq('id', id);
  ...
  return (
    <>
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <header className="flex items-center justify-between px-3 sm:px-4 md:px-6 lg:px-10 py-6 bg-[#0B0C10]/80 backdrop-blur-xl border-b border-red-500/10 z-30 flex-shrink-0 animate-fade-in-down">
        ...
        <main className="flex-1 flex overflow-hidden relative">
          <div className="flex-1 overflow-y-auto px-4 md:px-10 py-8 relative custom-scrollbar">
            ...
            <div className="absolute top-0 right-0 -mr-40 -mt-40 w-48 sm:w-72 md:w-96 h-48 sm:h-72 md:h-96 bg-red-600/10 rounded-full blur-[100px] pointer-events-none animate-orb-pulse"></div>
            <div className="absolute bottom-0 left-0 -ml-20 -mb-20 w-32 sm:w-48 md:w-64 h-32 sm:h-48 md:h-64 bg-red-900/10 rounded-full blur-[80px] pointer-events-none animate-float-slow"></div>
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-1 gap-6 pb-20">
                  {filteredHistory.map((meeting, index) => (
                    <div
                      key={meeting.id}
                      className="group bg-[#0B0C10]/60 backdrop-blur-md border border-white/10 rounded-2xl p-3 sm:p-4 md:p-6 transition-all hover:bg-[#0B0C10]/80 hover:border-white/20 hover:shadow-2xl hover-lift animate-fade-in-up"
          ...
          <aside className={`
            fixed xl:relative inset-y-0 right-0 w-[85vw] sm:w-80 md:w-96 bg-[#0B0C10] xl:bg-[#0B0C10]/80 backdrop-blur-md
            border-l border-white/5 px-3 sm:px-4 md:px-8 py-8 flex-col z-[70] md:z-20
              ...
              <div className="flex items-center justify-between bg-black/40 p-1 rounded-xl border border-white/5 mb-6">
                <button
                  onClick={() => setSelectedDate(
                    new Date(new Date(selectedDate || new Date()).setMonth((selectedDate || new Date()).getMonth() - 1))
                  )}
